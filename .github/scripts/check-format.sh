...
while IFS= read -r file; do
  ...
  if [[ ! -f "$file" || $(basename "$file") =~ yarn\.lock|package-lock\.json|.*\.spkg|.*\.svg|\.env.*|Dockerfile|Makefile ]]; then
    echo "Ignoring file: $file" >&2
    continue
  fi
